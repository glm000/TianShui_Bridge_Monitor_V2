      ...
      <div class="toolbar-wrapper">
        <div class="filter-area">
          ...
          <el-input 
            ...
            placeholder="搜索监测点位/内容..." 
            clearable 
            class="glass-input w-220"
        ...
        <div class="action-area">
           <transition name="el-fade-in">
            <div v-if="selectedIds.length > 0" class="batch-actions">
              <span class="selected-hint">已选 <span class="num">{{ selectedIds.length }}</span> 项</span>
              <el-button-group>
            ...
            <div class="detail-content-box">
                ...
                <div class="detail-grid-row">
                 ...
                 <div class="detail-grid-row">
                    <div class="detail-cell">
                        <label>当前数值</label>
                        <div class="val value-font lg">{{ formatValue(currentAlarm.val) }} <span class="unit">{{ currentAlarm.unit }}</span></div>
                    </div>
                    <div class="detail-cell">
                         <label>阈值范围</label>
...
const loadBridges = async () => {
  try { const res = await getBridgesWithSensors(); if(res.data?.success) bridges.value = res.data.data } catch(e){}
}
const loadStatistics = async () => {
    try { const res = await getAlarmStatistics({ bridgeId: filterForm.bridgeId }); if(res.data?.success) Object.assign(statistics, res.data.data.status) } catch(e){}
}
const loadAlarmList = async () => {
  loading.value = true
  try {
    const res = await getAlarmList({ ...filterForm, ...pagination })
    if (res.data?.success) { alarmList.value = res.data.data.list; pagination.total = res.data.data.total }
  } finally { loading.value = false }
}

// 事件处理
const handleQuery = () => { pagination.page = 1; loadAlarmList(); loadStatistics() }
const handleSelectionChange = (sel) => { selectedIds.value = sel.map(i => i.id) }
const handlePageChange = (p) => { pagination.page = p; loadAlarmList() }
const handleSizeChange = (s) => { pagination.pageSize = s; loadAlarmList() }
const handleViewDetail = async (row) => {
    const res = await getAlarmById(row.id); 
    if(res.data?.success) { currentAlarm.value = res.data.data; detailDialogVisible.value = true }
}
// 批量操作处理
  ...
  try {
    if (cmd === 'delete') {
      await ElMessageBox.confirm(
        `确认删除选中的 ${selectedIds.value.length} 条告警记录吗？此操作不可恢复。`,
        '批量删除确认',
...
onMounted(() => { loadBridges(); loadAlarmList(); loadStatistics() })
</script>
...
.stat-info .label { font-size: 12px; color: rgba(160, 180, 206, 0.8); margin-bottom: 2px; }
.stat-info .value { font-family: 'JetBrains Mono', monospace; font-size: 24px; font-weight: 700; color: #fff; }
.stat-info .unit { font-size: 12px; color: rgba(160, 180, 206, 0.6); margin-left: 4px; }

/* 卡片特定色 */
.total .stat-icon-wrapper { color: #40f3ff; background: rgba(64, 243, 255, 0.1); }
.total .card-glow { position: absolute; right: 0; top: 0; width: 60px; height: 60px; background: radial-gradient(circle, rgba(64,243,255,0.2) 0%, transparent 70%); pointer-events: none; }
.pending .stat-icon-wrapper { color: #fa8c16; background: rgba(250, 140, 22, 0.1); }
.pending .value { color: #fa8c16; }
.processing .stat-icon-wrapper { color: #597ef7; background: rgba(89, 126, 247, 0.1); }
.resolved .stat-icon-wrapper { color: #52c41a; background: rgba(82, 196, 26, 0.1); }
...
.icon-btn-glass:hover { background: rgba(64, 243, 255, 0.1); color: #fff; border-color: #40f3ff; }
...
:deep(.el-table th) { font-weight: 600; color: rgba(160,180,206,1); border-bottom: 1px solid rgba(64, 243, 255, 0.15) !important; }
:deep(.el-table td) { border-bottom: 1px solid rgba(64, 243, 255, 0.05) !important; }
...
.text-sub { color: rgba(255,255,255,0.45); font-size: 12px; }
...
.code-font { font-family: monospace; color: rgba(160,180,206,0.8); font-size: 14px; }
...
.more-btn { cursor: pointer; color: rgba(255,255,255,0.6); transition: 0.2s; font-size: 16px; }
.more-btn:hover { color: #40f3ff; }
...
:global(.glass-dialog .el-dialog__title) { color: #fff !important; }
:global(.glass-dialog .el-dialog__headerbtn .el-dialog__close) { color: #fff; }
...
/* 详情弹窗内部布局 */
.detail-container { color: #fff; }
.detail-header { display: flex; justify-content: space-between; align-items: center; margin-bottom: 20px; padding-bottom: 12px; border-bottom: 1px solid rgba(255,255,255,0.1); }
.dh-left .id-tag { background: rgba(255,255,255,0.1); padding: 2px 8px; border-radius: 4px; font-family: monospace; margin-right: 12px; color: #40f3ff; }
.dh-left .time { color: rgba(255,255,255,0.5); font-size: 13px; }

.detail-content-box { background: rgba(0,0,0,0.2); padding: 16px; border-radius: 6px; border: 1px solid rgba(255,255,255,0.05); }
.detail-row.highlight { margin-bottom: 16px; }
.detail-row label { display: block; font-size: 12px; color: rgba(255,255,255,0.5); margin-bottom: 4px; }
.detail-row .val.text-danger { color: #ff7875; background: rgba(255, 77, 79, 0.1); padding: 8px 12px; border-radius: 4px; border: 1px solid rgba(255, 77, 79, 0.2); }

.detail-grid-row { display: grid; grid-template-columns: 1fr 1fr; gap: 20px; margin-bottom: 12px; }
.detail-grid-row:last-child { margin-bottom: 0; }
.detail-cell label { font-size: 12px; color: rgba(255,255,255,0.4); }
.detail-cell .val { font-size: 15px; color: #fff; border-bottom: 1px solid rgba(255,255,255,0.1); padding-bottom: 4px; }
.detail-cell .val.lg { font-size: 18px; color: #40f3ff; border-bottom: none; }

.detail-footer { margin-top: 24px; text-align: right; padding-top: 16px; border-top: 1px solid rgba(255,255,255,0.1); }
:deep(.el-overlay) {
