...
exports.login = async (req, res) => {
  try {
    ...
    // 3. 记录登录日志
    await db.query('INSERT INTO sys_logs (user_id, username, action, ip_addr, details) VALUES (?, ?, ?, ?, ?)', 
      [user.id, user.username, 'LOGIN', req.ip, '用户登录成功'])
...
exports.getActionTypes = async (req, res) => {
  try {
    const [rows] = await db.query(
      'SELECT DISTINCT action FROM sys_logs WHERE action IS NOT NULL ORDER BY action'
    )
...
exports.getLogUsers = async (req, res) => {
  try {
    const [rows] = await db.query(
      'SELECT DISTINCT username FROM sys_logs WHERE username IS NOT NULL ORDER BY username'
    )
...
exports.getLogStatistics = async (req, res) => {
  try {
    ...
    const [todayCount] = await db.query(
      'SELECT COUNT(*) as count FROM sys_logs WHERE DATE(created_at) = CURDATE()'
    )

    // 最近登录次数（24小时）
    const [loginCount] = await db.query(
      "SELECT COUNT(*) as count FROM sys_logs WHERE action = 'LOGIN' AND created_at > DATE_SUB(NOW(), INTERVAL 24 HOUR)"
    )
...
exports.deleteLog = async (req, res) => {
  try {
    ...
    await db.query(
      ...
      [userId, username, 'DELETE_LOG', req.ip, `删除日志ID: ${id}`]
    )
...
exports.batchDeleteLogs = async (req, res) => {
  try {
    ...
    await db.query(
      'INSERT INTO sys_logs (user_id, username, action, ip_addr, details) VALUES (?, ?, ?, ?, ?)',
      [userId, username, 'BATCH_DELETE_LOG', req.ip, `批量删除 ${result.affectedRows} 条日志`]
    )
